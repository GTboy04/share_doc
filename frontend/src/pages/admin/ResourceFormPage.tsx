...
export function ResourceFormPage() {
  ...
  useEffect(() => {
    void api.listAdminCategories().then((result) => setCategories(result.items));
    if (id) {
  ...
  const updateLink = <K extends keyof EditableLink>(index: number, key: K, nextValue: EditableLink[K]) => {
    setFormData((value) => ({
      ...value,
      links: value.links.map((item, itemIndex) => (itemIndex === index ? { ...item, [key]: nextValue } : item)),
    }));
  };
  ...
  const removeLink = (index: number) => {
    setFormData((value) => ({
      ...value,
      links: value.links.filter((_, itemIndex) => itemIndex !== index).map((item, itemIndex) => ({ ...item, sort_order: itemIndex })),
    }));
  };
  ...
  return (
    <div className="page-stack page-stack--compact">
      <PageHero
        eyebrow={isEdit ? "Resource Editing" : "Resource Creation"}
      ...
      <Card className="admin-page-card">
        <form className="simple-form admin-form-grid" onSubmit={onSubmit}>
          ...
          <Input value={formData.title} onChange={(event) => setFormData((value) => ({ ...value, title: event.target.value }))} placeholder="请输入资源标题" />
          <div className="admin-form-grid__split">
            <div>
              <label className="field-label">分类</label>
              <select className="field-select" value={formData.category_id} onChange={(event) => setFormData((value) => ({ ...value, category_id: event.target.value }))}>
            ...
            <div>
              ...
              <Input
                value={formData.year}
                onChange={(event) => setFormData((value) => ({ ...value, year: event.target.value.replace(/[^\d]/g, "").slice(0, 4) }))}
                placeholder="例如：2027"
                inputMode="numeric"
              />
            </div>
          ...
          <Input value={formData.tags} onChange={(event) => setFormData((value) => ({ ...value, tags: event.target.value }))} placeholder="多个标签用逗号隔开" />
          <label className="field-label">描述</label>
          <Input.TextArea rows={5} value={formData.description} onChange={(event) => setFormData((value) => ({ ...value, description: event.target.value }))} />
          <div className="links-editor">
            ...
            {formData.links.length === 0 ? (
              <div className="links-editor__empty">暂未添加链接项。可按平台类型逐条添加，链接地址可先留空。</div>
            ) : null}
            {formData.links.map((link, index) => (
              <div key={`${index}-${link.sort_order}`} className="link-editor-card">
                <div className="admin-form-grid__split">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(event) => updateLink(index, "platform_type", event.target.value as ResourceLinkPlatform)}
                    >
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) => updateLink(index, "custom_title", event.target.value)}
                      placeholder="例如：夸克备用 1"
                    />
                  </div>
                </div>
                <label className="field-label">链接地址</label>
                <Input value={link.url} onChange={(event) => updateLink(index, "url", event.target.value)} placeholder="https://...（可留空）" />
                <Space className="link-editor-card__actions">
                  <Button danger htmlType="button" onClick={() => removeLink(index)}>
                    删除该链接
                  </Button>
                </Space>
              </div>
