...
export function RequestManagementPage() {
  ...
  const updateStatus = async (item: UserRequest, nextStatus: RequestStatus) => {
    try {
      await api.updateAdminRequest(item.id, {
        status: nextStatus,
  ...
  const canNotify = (item: UserRequest) =>
    item.status === "done" && Boolean(item.contact_email);
        ...
        <div className="admin-filter-panel">
          ...
          <Space wrap className="admin-filters">
            <select
              ...
              value={status ?? ""}
              onChange={(event) =>
                setStatus((event.target.value as RequestStatus) || undefined)
              }>
                  ...
                  <td>
                    <Space wrap>
                      ...
                      <Button
                        size="small"
                        danger
                        onClick={() => void updateStatus(item, "ignored")}>
                        忽略
                      </Button>
                      <Button
                        size="small"
                        disabled={!canNotify(item)}
                        onClick={() => void notify(item)}>
                        发送通知
                      </Button>
                    </Space>
                  </td>
