      ...
      <Card className="admin-page-card">
        <div className="admin-filter-panel">
          ...
          <Space wrap className="admin-filters">
            <Input placeholder="搜索标题、描述、标签" value={keyword} onChange={(event) => setKeyword(event.target.value)} />
            <select
            ...
            <Input
              ...
              inputMode="numeric"
            />
            <select className="field-select" value={status ?? ""} onChange={(event) => setStatus(event.target.value || undefined)}>
            ...
            <tbody>
              {items.map((record) => (
                <tr key={record.id}>
                  ...
                  <td>{new Date(record.updated_at).toLocaleDateString("zh-CN")}</td>
                  <td>
                    <Space>
                      <Link to={`/admin/resources/${record.id}/edit`}>编辑</Link>
                      <Button type="link" danger onClick={() => void remove(record.id)}>
                        删除
                      </Button>
                    </Space>
                  </td>
